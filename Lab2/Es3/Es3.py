...
from GroundTrackPerturbed import GroundTrackPerturbed
from RepeatingGroundTrackPerturbed import RepeatingGroundTrackPerturbed
...
# case 1: generic orbit with k=12, m=1
a_1 = 8350  # [km]
e_1 = 0.19760  # [-]
...
# computing the new semi-major axis
a_1 = RepeatingGroundTrackPerturbed(s0_1, 12, 1, muP, wP, j2, 2)
print(a_1)
s0_1[0] = a_1
...
# time
n_orbits_1 = 20
T_1 = 2 * np.pi / np.sqrt(muP / a_1**3)
t_1 = np.linspace(0, n_orbits_1 * T_1, k)

# computing the parameters
alpha_1, delta_1, lon_1, lat_1 = GroundTrackPerturbed(s0_1, lon0_1, t_1, muP, wP, RP, j2, 2)
...
# case 2: Molniya orbit with k = 2, m = 1
a_2 = 26600  # [km]
e_2 = 0.74  # [-]
...
#computing the new semi-major axis
a_2 = RepeatingGroundTrackPerturbed(s0_2, 2, 1, muP, wP, j2, 2)
print(a_2)
s0_2[0] = a_2

# time
n_orbits_2 = 30
T_2 = 2 * np.pi / np.sqrt(muP / a_2**3)
t_2 = np.linspace(0, n_orbits_2 * T_2, k)

# computing the parameters
alpha_2, delta_2, lon_2, lat_2 = GroundTrackPerturbed(s0_2, lon0_2, t_2, muP, wP, RP, j2, 2)
...
k_vec = np.array([20, 29, 15])
m_vec = np.array([2, 2, 1])
...
for j in np.arange((len(i_3))):
    ...
    a_3 = RepeatingGroundTrackPerturbed(s0_3[j, :], k_vec[j], m_vec[j], muP, wP, j2, 2)
    print(a_3)
    s0_3[j, 0] = a_3
    ...
    # computing the parameters
    alpha_3[j, :], delta_3[j, :], lon_3[j, :], lat_3[j, :] = GroundTrackPerturbed(s0_3[j, :], lon0_3, t_3, muP, wP, RP, j2, 2)
