...
j2 = 0.0010826356274431847715527733655357   # j2 second zonal harmonic [-]

# Initial conditions

# set 1: Quasi circular Medium Earth Orbit
# r0 = np.array([26578.137, 0, 0])  # [km]
# v0 = np.array([0, 2.221, 3.173])  # [km/s]

# set 2: Highly eccentric and inclined orbit
r0 = np.array([6495, -970, -3622])  # [km]
v0 = np.array([4.752, 2.130, 7.950])  # [km/s]

y0 = np.concatenate((r0, v0))  # [km, km/s]
...
t = np.linspace(0, n*T, 50000)  # propagating for 1 year (3.156e+7s), in alternative set n*T
...
# Generate the orbit color sequence, from green to yellow
'''
starting color is green which is represented by the RGB value (0, 1, 0), and the ending color is yellow which is 
represented by the RGB value (1, 1, 0). The n parameter determines the number of colors in the sequence. 
The np.linspace function returns an array of evenly spaced values between the start and end RGB values.
'''
color_seq = np.linspace((0, 1, 0), (1, 1, 0), n)

# Plot orbits with color sequence
s = k/n
s = int(s)

for ii in range(n):
    color = color_seq[ii % n]  # get color from color sequence
    ax1.plot(r[(ii + (ii-1)*s): n*s, 0], r[(ii + (ii-1)*s): n*s, 1], r[(ii + (ii-1)*s): n*s, 2], color=color)

ax1.plot(r[0:s, 0], r[0:s, 1], r[0:s, 2], 'b', label='Initial Trajectory')  # this is the initial orbit in blue
...
# norm object: takes into account the min and the max value, which will be used to map values in the range [0, n] to
# colors in the colormap.
norm = mpl.colors.Normalize(vmin=0, vmax=n)

# cmap object: takes into account the colors of the colorbar, mapping a list of colors to a sequence of evenly
# spaced values. color_seq is a list of colors that will be used to create the colormap.
cmap = mpl.colors.ListedColormap(color_seq)

# mappable object: maps values to colors based on the norm and cmap objects defined above. The mappable object will be
# used to create the colorbar.
mappable = mpl.cm.ScalarMappable(norm=norm, cmap=cmap)

# define the locations and labels of the ticks on the colorbar.
tick_locs = [0, n/2, n]
tick_labels = ['0', f'{n/2}', f'{n}']

# create the colorbar object itself and add the labels
cbar = plt.colorbar(mappable, ax=ax1, orientation='vertical', ticks=tick_locs, location='left')
